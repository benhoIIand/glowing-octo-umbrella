...
module.exports = (job, currentTime) => {
  ...
  if (isHourWild && isMinuteWild) {
    return `${padNumber(targetTime.hour)}:${padNumber(targetTime.minute)} today - ${job.cmd}`;
  }
  ...
  if (runHour < targetTime.hour) {
    return `${padNumber(runHour)}:${padNumber(runMinute)} tomorrow - ${job.cmd}`;
  }

  const hour = runMinute > targetTime.minute ? targetTime.hour : runHour;

  // Run today
  return `${padNumber(runHour)}:${padNumber(runMinute)} today - ${job.cmd}`;
};
